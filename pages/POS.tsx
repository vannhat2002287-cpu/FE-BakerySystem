          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-20">
              {filteredProducts.map((product) => {
                const stock =
                  inventory.find((i) => i.product_id === product.product_id)
                    ?.current_quantity || 0;
                const isStockManaged =
                  product.type !== "drink" && product.type !== "alcohol";

                const isOutOfStock = isStockManaged && stock <= 0;
                const isAlcoholRestricted =
                  product.is_alcoholic && !isAlcoholAllowed;
                  ...
                  >
                    <div className="aspect-square bg-gray-100 relative">
                      ...
                      {isStockManaged && stock > 0 && (
                        <span
                          className={`absolute top-2 right-2 text-xs font-bold px-2 py-1 rounded-full shadow-sm z-10 ${
                            stock <= 5
            ...
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                orderType === "eat-in"
                  ...
                  : "text-gray-500"
              } ${!isEatInAllowed ? "opacity-50 cursor-not-allowed" : ""}`}
            >
              店内 (Eat-in)
              {!isEatInAllowed && (
