...
const POS: React.FC = () => {
  const { products, inventory, cart, addToCart, removeFromCart, updateCartQuantity, clearCart, placeOrder, currentTime } = useStore();
        ...
        <div className="p-4 bg-white border-b border-gray-200 shadow-sm z-10">
          ...
          <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
            <button 
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${selectedCategory === 'all' ? 'bg-brand-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              すべて
            </button>
            {CATEGORIES.map(cat => (
              <button 
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${selectedCategory === cat.category_id ? 'bg-brand-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
                {cat.name}
              </button>
            ))}
